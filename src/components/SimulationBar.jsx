...
import { Flame, Wind, AlertTriangle, RefreshCw, CheckCircle2, Wifi, WifiOff, Database, Watch, Droplets, Moon, HeartPulse, SlidersHorizontal, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 border-b border-slate-800/80 px-4 py-1.5 text-[11px] flex flex-wrap items-center justify-between gap-2 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 font-extrabold tracking-wider uppercase text-[10px] border border-purple-500/30 flex items-center gap-1">
          AI + IoT SYSTEM
        </span>
        ...
        <button
          ...
          className="ml-2 px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 font-bold flex items-center gap-1 text-[10px]"
        >
          <SlidersHorizontal className="w-3 h-3 text-teal-400" />
          <span>Demo Controls</span>
          {showDemoControls ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
        </button>
      </div>
